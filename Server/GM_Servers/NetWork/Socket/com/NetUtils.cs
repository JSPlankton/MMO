...
namespace NetWork.Socket.com
{
    public class NetUtils : Singleton<NetUtils>
    {
        ...
        public ushort CRC16(byte[] body)  //CRC校验 
        {
            byte CRCHigh = 0xFF; //重置CRC高位校验码
            byte CRCLow = 0xFF; //重置CRC低位校验码
            for (int i = 0; i < body.Length; i++)
            {
                int crcIndex = CRCHigh ^ body[i];
        ...
        public byte[] Compress(byte[] data)
        {
            ...
            {
                using (GZipStream zipStream = new GZipStream(compStream, CompressionMode.Compress))
                {
                    zipStream.Write(data, 0, data.Length);
                    zipStream.Close();  //必须关闭 否则无法解压数据
